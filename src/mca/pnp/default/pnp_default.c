...
static int default_init(void)
{
    ...
    if (!recv_on) {
        ...
        if (ORCM_PROC_IS_MASTER || ORCM_PROC_IS_DAEMON || ORCM_PROC_IS_TOOL) {
            ...
            if (ORTE_SUCCESS != (ret = orte_rmcast.recv_buffer_nb(triplet->channel, request->tag,
                          ...
                          orcm_pnp_callback_fn_t cbfunc)
{
    ...
    /* setup to listen to it - will just return if we already are */
    if (ORTE_SUCCESS != (ret = orte_rmcast.recv_nb(triplet->channel, tag,
                                                   ORTE_RMCAST_PERSISTENT,
                                                   recv_inputs, cbfunc))) {
        ORTE_ERROR_LOG(ret);
    }
                          ...
                          struct iovec *msg, int count)
{
    ...
    if (NULL == recipient ||
        ...
        if (ORCM_SUCCESS != (ret = orte_rmcast.send(chan, tag, msg, count))) {
            ORTE_ERROR_LOG(ret);
        }
        OPAL_RELEASE_THREAD(&lock, &cond, &active);
        return ORCM_SUCCESS;
                             ...
                             void *cbdata)
{
    ...
    if (NULL == recipient ||
        ...
        /* send the iovecs to the channel */
        if (ORCM_SUCCESS != (ret = orte_rmcast.send_nb(chan, tag, msg, count,
                                 ...
                                 opal_buffer_t *buffer)
{
    ...
    if (NULL == recipient ||
        ...
        OPAL_OUTPUT_VERBOSE((2, orcm_pnp_base.output,
                             ...
                             chan, tag));
        if (ORCM_SUCCESS != (ret = orte_rmcast.send_buffer(chan, tag, buffer))) {
            ORTE_ERROR_LOG(ret);
        }
        OPAL_RELEASE_THREAD(&lock, &cond, &active);
        return ORCM_SUCCESS;
                                    ...
                                    void *cbdata)
{
    ...
    if (NULL == recipient ||
        ...
        if (ORCM_SUCCESS != (ret = orte_rmcast.send_buffer_nb(chan, tag, buffer,
                               ...
                               opal_buffer_t *buf, void *cbdata)
{
    ...
    for (i=0; i < triplets.size; i++) {
        ...
        if (0 == strcasecmp(triplet->string_id, string_id)) {
            free(string_id);
...
static void recv_inputs(int status,
                        ...
                        struct iovec *msg, int count, void *cbdata)
{
    orcm_pnp_triplet_t *triplet, *trp;
    ...
    /* the rmcast channel is associated with a triplet - get it */
    trp = NULL;
    for (i=0; i < triplets.size; i++) {
        if (NULL == (triplet = (orcm_pnp_triplet_t*)opal_pointer_array_get_item(&triplets, i))) {
            continue;
        }
        if (channel == triplet->channel) {
            trp = triplet;
            break;
        }
    }
    ...
    if (NULL == (request = find_request(&trp->requests, tag))) {
                                       ...
                                       char *release)
{
    ...
    i = opal_pointer_array_add(&triplets, triplet);
    
    OPAL_OUTPUT_VERBOSE((2, orcm_pnp_base.output,
                         "%s pnp:default:get_triplet created triplet channel %d status %d",
                         ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                         triplet->channel, i));
