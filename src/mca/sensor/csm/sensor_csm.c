...
orcm_sensor_base_module_t orcm_sensor_csm_module = {
    init,
    finalize,
    start,
    stop
...
static void start(void)
{
    if (!sampling && 0 < mca_sensor_csm_component.sample_rate) {
        /* startup a timer to wake us up periodically
         * for a data sample
         */
        sampling = true;
        ORTE_TIMER_EVENT(mca_sensor_csm_component.sample_rate, 0, sample);
    }
    return;
}
...
static void sample(int fd, short event, void *arg)
{
    ...
    OPAL_OUTPUT_VERBOSE((0, orcm_sensor_base_output,
                         "sample:csm sampling sensors"));
    ...
    if (celsius > mca_sensor_csm_component.celsius_limit) {
        OPAL_OUTPUT_VERBOSE((0, orcm_sensor_base_output,
                             "sample:csm got temperature of %3.2f celsius, over-limit",
                             celsius));
        /* we should notify the CM to not schedule new jobs to this node */
    } else {
        OPAL_OUTPUT_VERBOSE((0, orcm_sensor_base_output,
                             "sample:csm got temperature of %3.2f celsius",
                             celsius));
    ...
    for (item = opal_list_get_first(&orte_local_children);
        ...
        OPAL_OUTPUT_VERBOSE((0, orcm_sensor_base_output,
                             "sample:csm got cpu time of %lu seconds for proc %s",
