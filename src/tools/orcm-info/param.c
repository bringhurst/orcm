...
void orcm_info_do_params(bool want_all_in, bool want_internal)
{
    ...
    if (want_all) {
        for (i = 0; i < mca_types.size; ++i) {
            ...
            orcm_info_show_mca_params(info, type, orcm_info_component_all, want_internal);
        }
    } else {
        for (i = 0; i < count; ++i) {
            type = opal_cmd_line_get_param(orcm_info_cmd_line, "param", (int)i, 0);
            ...
            orcm_info_show_mca_params(info, type, component, want_internal);
        }
    }
...
void orcm_info_do_path(bool want_all, opal_cmd_line_t *cmd_line)
{
    ...
    if (want_all) {
        ...
        orcm_info_show_path(orcm_info_path_pkglibdir, opal_install_dirs.pkglibdir);
        ...
        orcm_info_show_path(orcm_info_path_pkgdatadir, opal_install_dirs.pkgdatadir);
        orcm_info_show_path(orcm_info_path_pkglibdir, opal_install_dirs.pkglibdir);
        orcm_info_show_path(orcm_info_path_pkgincludedir, opal_install_dirs.pkgincludedir);
    } else {
        count = opal_cmd_line_get_ninsts(cmd_line, "path");
        for (i = 0; i < count; ++i) {
            scope = opal_cmd_line_get_param(cmd_line, "path", i, 0);
