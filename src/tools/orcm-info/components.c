 ...
 */
void orcm_info_open_components(void)
{
    int i;
    char *env, *str;
    ...
    /* ORCM frameworks */
    if (ORTE_SUCCESS != orcm_cfgi_base_open()) {
        goto error;
    }
    map = OBJ_NEW(orcm_info_component_map_t);
    ...
    if (ORTE_SUCCESS != orcm_clip_base_open()) {
        goto error;
    }
    map = OBJ_NEW(orcm_info_component_map_t);
    ...
    if (ORTE_SUCCESS != orcm_leader_base_open()) {
        goto error;
    }
    map = OBJ_NEW(orcm_info_component_map_t);
    ...
    if (ORTE_SUCCESS != orcm_pnp_base_open()) {
        goto error;
    }
    map = OBJ_NEW(orcm_info_component_map_t);
...
error:
    fprintf(stderr, "%s\n", str);
    fprintf(stderr, "orcm-info will likely not display all configuration information\n");
...
void orcm_info_close_components()
{
    int i;
    orcm_info_component_map_t *map;
    ...
    if (opened_components) {
        ...
        (void) orcm_pnp_base_close();
        (void) orcm_leader_base_close();
        (void) orcm_clip_base_close();
        (void) orcm_cfgi_base_close();
