...
/*
 * Callback when non-blocking send completes.
 */
...
void orte_iof_orcmd_read_handler(int fd, short event, void *cbdata)
{
    ...
    OPAL_OUTPUT_VERBOSE((1, orte_iof_base.iof_output,
                         "%s iof:orcmd:read handler read %d bytes from %s, fd %d",
    ...
    if (numbytes <= 0) {
        if (0 > numbytes) {
            ...
            OPAL_OUTPUT_VERBOSE((1, orte_iof_base.iof_output,
                                 "%s iof:orcmd:read handler %s Error on connection:%d",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
    ...
    for (item = opal_list_get_first(&mca_iof_orcmd_component.procs);
        ...
        if (proct->name.jobid == rev->name.jobid &&
            ...
            if (NULL == proct->revstdout &&
                ...
                opal_list_remove_item(&mca_iof_orcmd_component.procs, item);
