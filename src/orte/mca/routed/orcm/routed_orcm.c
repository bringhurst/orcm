    ...
    {  /* MUST BE A PROC */
        ...
        if (NULL != ndat) {
            ...
            if (ORTE_JOB_FAMILY(ORTE_PROC_MY_NAME->jobid) != ORTE_JOB_FAMILY(job)) {
                 ...
                 */
                OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                                     "%s routed_orcm_init_routes: diff job family - sending update to %s",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                ...
                OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                                     "%s routed_orcm_init_routes: ack recvd",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
...
static int route_lost(const orte_process_name_t *route)
{
    ...
    if (!orte_finalizing &&
        ...
        opal_output(0, "%s routed:orcm: Connection to lifeline %s lost",
                    ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
...
static orte_vpid_t get_routing_tree(opal_list_t *children)
{
    ...
    /* for the HNP, the orcm routing tree is direct to all known alive daemons */
    if (NULL != children) {
